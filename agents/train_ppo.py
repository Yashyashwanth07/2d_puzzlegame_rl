    ...
    def _on_step(self):
        ...
        if self.locals.get('dones') is not None:
            for i, done in enumerate(self.locals['dones']):
                if done:
                    # Access info dict to get episode info if available
...
def main():
    parser = argparse.ArgumentParser(description='Train PPO DDA Director')
    parser.add_argument('--config', default='configs/ppo_config.yaml', help='Path to config file')
    ...
    log_dir = os.path.join(config.get('log_dir', 'results/logs/ppo'), timestamp)
    model_dir = os.path.join(config.get('model_dir', 'results/models/ppo'), timestamp)
    os.makedirs(log_dir, exist_ok=True)
    ...
    # Save final model
    final_path = os.path.join(model_dir, 'ppo_dda_final')
